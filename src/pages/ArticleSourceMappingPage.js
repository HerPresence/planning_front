    ...
    <Modal title="Створити нову master-статтю зі staging" onClose={onClose} size="large">
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-grid">
          ...
          <div className="form-field"><label>Level 1</label><input name="level1" value={form.level1} onChange={handleChange} /></div>
          <div className="form-field"><label>Level 2</label><input name="level2" value={form.level2} onChange={handleChange} /></div>
          <div className="form-field full">
...
function ActionsCell({ row, saving, onClear, onReject, onCreate }) {
  const isMapped = !!row.master_article_id;
  const isSaving = saving[`${row.source_id}__${row.source_article_id}`];
